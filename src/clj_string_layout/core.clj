...
(defn ^:private make-col-layout-parser
  "constructs an instaparse parser for rows based on grammar"
  []
  (insta/parser (str grammar \newline "align = ('L'|'C'|'R'|'V')")
                :string-ci true))
(def ^:private make-col-layout-parser-m (memoize make-col-layout-parser))

(defn ^:private make-row-layout-parser
  "constructs an instaparse parser for cols based on grammar"
  []
  (insta/parser (str grammar \newline "align = #'[^]]'")
                :string-ci true))
(def ^:private make-row-layout-parser-m (memoize make-row-layout-parser))
